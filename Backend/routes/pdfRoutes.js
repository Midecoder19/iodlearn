...
router.get("/download/:branch/:year/:semester/:subjectCode/:unit", async (req, res) => {
  ...
  const publicId = `iodlearn_notes/Notes/${branch}/${year}/${semester}/${subjectCode}/Unit_${unit}/${subjectCode}-unit-${unit}.pdf`;
  const fallbackPublicId = `jit_learning_notes/Notes/${branch}/${year}/${semester}/${subjectCode}/Unit_${unit}/${subjectCode}-unit-${unit}.pdf`;
  ...
  try {
    ...
    console.warn("⚠️ Cloudinary fetch error for new path; trying old path", error.message);
  }

  try {
    const fallbackResource = await cloudinary.api.resource(fallbackPublicId, {
      resource_type: "raw",
    });
    return res.redirect(fallbackResource.secure_url);
  } catch (error) {
...
router.get("/subject-catalog/:branch/:year/:semester/:subject", async (req, res) => {
    ...
    const folderPrefix = `iodlearn_notes/Notes/${branch}/${year}/${semester}/${subject}/`;
    ...
    try {
        ...
        // Fallback: If no results, try without the leading 'iodlearn_notes/' if it was omitted in upload config
        if (resources.length === 0 && folderPrefix.startsWith("iodlearn_notes/")) {
            const fallbackPrefix = folderPrefix.replace("iodlearn_notes/", "");
